...
SAMPLE CHANGES TO OPENPIFPAF.VIDEO THAT UTILIZES THE TRACKER AND THE STORAGE
"""
...
from . import decoder, logger, network, show, visualizer, track_recover, __version__
...
def cli():  # pylint: disable=too-many-statements,too-many-branches
    ...
    parser.add_argument('--disable-cuda', action='store_true',
                        help='disable CUDA')
    parser.add_argument('--reid-model', default=None, nargs='?', const=True,
                        help='reid model file')
...
def main():
    ...
    start = time.time()
    print('Creating storage and ID tracker...')
    extractor = track_recover.FeatureExtractor(model_path)
    storage = track_recover.Storage(extractor=extractor)
    id_tracker = track_recover.IDTracker(sequence=args.source, verbose=True)

    print('Storage and tracker created successfully in {:.4f}'.format(time.time() - start))
    ...
    for (ax, ax_second), (preds, _, meta), (image, _, _, _) in \
            zip(animation.iter(), predictor.dataset(capture), capture):

        annotations = [ann.json_data() for ann in preds]
        storage.update_frame(image, annotations)

        frame_id, input_IDs = storage.get_last_frame()

        id_tracker.update(input_IDs=input_IDs, frame=frame_id)
        id_tracker.modify_annotations(annotations=preds, frame=frame_id)
